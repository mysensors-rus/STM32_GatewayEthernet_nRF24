...
//  MY_SPI.setClockDivider(SPI_CS, 21);
//  MY_SPI.setClockDivider(SPI_CS, 6); // 14 Mhz, ok  
//  MY_SPI.setClockDivider(SPI_CS, 3); // 28 Mhz, ok 
  MY_SPI.setClockDivider(SPI_CS, 2); // 42 Mhz, ok 
  MY_SPI.setDataMode(SPI_CS, SPI_MODE0);
#endif
...
#if defined(ARDUINO_ARCH_AVR)
    setSS();  
    MY_SPI.transfer(_addr >> 8);
    MY_SPI.transfer(_addr & 0xFF);
    MY_SPI.transfer(_cb);
    MY_SPI.transfer(_data);
    resetSS();
#elif defined(__STM32F1__)
...
#else
  MY_SPI.transfer(SPI_CS, _addr >> 8, SPI_CONTINUE);
  MY_SPI.transfer(SPI_CS, _addr & 0xFF, SPI_CONTINUE);
  MY_SPI.transfer(SPI_CS, _cb, SPI_CONTINUE);
  MY_SPI.transfer(SPI_CS, _data);
#endif    
    return 1;
}
...
uint16_t W5500Class::write(uint16_t _addr, uint8_t _cb, const uint8_t *_buf, uint16_t _len)
{
  ...
  MY_SPI.transfer(SPI_CS, _cb, SPI_CONTINUE);
  for (i=0; i<_len-1; i++){
    MY_SPI.transfer(SPI_CS, _buf[i], SPI_CONTINUE);
  }
	MY_SPI.transfer(SPI_CS, _buf[i]);
...
uint8_t W5500Class::read(uint16_t _addr, uint8_t _cb)
{
    ...
    MY_SPI.transfer(_addr & 0xFF);
    MY_SPI.transfer(_cb);
    uint8_t _data = MY_SPI.transfer(0);
    resetSS();
#elif defined(__STM32F1__)
  digitalWrite(STM32_SPI_CS, LOW);
  MY_SPI.transfer( _addr >> 8);
  ...
  digitalWrite(STM32_SPI_CS, HIGH);
    ...
    MY_SPI.transfer(SPI_CS, _addr & 0xFF, SPI_CONTINUE);
    MY_SPI.transfer(SPI_CS, _cb, SPI_CONTINUE);
    uint8_t _data = MY_SPI.transfer(SPI_CS, 0);
#endif    
    return _data;
}
...
{ 
  ...
  digitalWrite(STM32_SPI_CS, HIGH);
    ...
    MY_SPI.transfer(SPI_CS, _cb, SPI_CONTINUE);
  for (i=0; i<_len-1; i++){
    _buf[i] = MY_SPI.transfer(SPI_CS, 0, SPI_CONTINUE);
  }
    _buf[_len-1] = MY_SPI.transfer(SPI_CS, 0);
